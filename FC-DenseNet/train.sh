...
test_every_n_step=500
batch_size=2

# where is your training lmdb database
train_database="/path/to/your/train.lmdb"
test_database="/path/to/your/test.lmdb"

output_folder="/path/to/where/to/save/the/model"
...
python train.py --test_every_n_steps=${test_every_n_step} --batch_size=${batch_size} --train_database=${train_database} --test_database=${test_database} --output_dir=${output_folder} --number_classes=${number_classes} --learning_rate=${learning_rate}  --use_augmentation=${use_augmentation} --balance_classes=${balance_classes}
