...
def enforce_size_multiple(img):
    ...
    factor = model.FCDensenet.SIZE_FACTOR
...
def process_slide_tiling(img, msk, tile_size, block_key):
    ...
    height = img.shape[0]
    width = img.shape[1]
    delta = int(tile_size - model.FCDensenet.RADIUS)
    if delta < 64:
        delta = int(0.8 * tile_size)
...
def generate_database(img_list, database_name, image_filepath, mask_filepath, output_folder, tile_size, image_format):
    ...
    for i in range(len(img_list)):
        ...
        block_key = img_file_name.replace('.{}'.format(image_format),'')
...
def main(image_folder, mask_folder, output_folder, dataset_name, train_fraction, image_format, use_tiling, tile_size):
    ...
    if not use_tiling:
        ...
        assert tile_size % model.FCDensenet.SIZE_FACTOR == 0, 'FCDensenet requires tiles with shapes that are multiples of 32'
    ...
    generate_database(train_img_files, database_name, image_folder, mask_folder, output_folder, tile_size, image_format)

    print('building test database')
    database_name = 'test-{}.lmdb'.format(dataset_name)
    generate_database(test_img_files, database_name, image_folder, mask_folder, output_folder, tile_size, image_format)
