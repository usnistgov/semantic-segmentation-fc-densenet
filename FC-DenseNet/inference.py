...
def _inference_tiling(img, fcd_model, tile_size):
    ...
    pad_x = 0
    pad_y = 0
    if img.shape[0] % model.FCDensenet.SIZE_FACTOR != 0:
        pad_y = (model.FCDensenet.SIZE_FACTOR - img.shape[0] % model.FCDensenet.SIZE_FACTOR)
        print('image height needs to be a multiple of {}, padding with reflect'.format(model.FCDensenet.SIZE_FACTOR))
    if img.shape[1] % model.FCDensenet.SIZE_FACTOR != 0:
        pad_x = (model.FCDensenet.SIZE_FACTOR - img.shape[1] % model.FCDensenet.SIZE_FACTOR)
        print('image width needs to be a multiple of {}, padding with reflect'.format(model.FCDensenet.SIZE_FACTOR))
    ...
    # radius = model.FCDensenet.RADIUS  # theoretical radius
    radius = fcd_model.estimate_radius()

    assert tile_size % model.FCDensenet.SIZE_FACTOR == 0
    assert radius % model.FCDensenet.SIZE_FACTOR == 0
